...
<div class="modal-body">
	<form class="form-horizontal" id="assignForm">
		...
		<input type="hidden" name="salesmanName" />
...
<script type="text/javascript">
	$('#assignForm').on('change', 'select[name=salesmanId]', function() {
		var salesmanName = $(this).find('option:selected').text();
		$('#assignForm input[name=salesmanName]').val(salesmanName);
	})
	$('#assignForm').submit(function(event) {
		event.preventDefault();
		$.ajax({
			url: '${baseUrl}/admin/customer/assign',
			data: $(this).serialize(),
			dataType: 'json',
			type: "post",
			async: true,
