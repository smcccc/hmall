	...
	<body class="gray-bg">
					...
					<div class="row">
						<form class="form-horizontal" id="permForm">
							...
							<div class="form-group">
								...
								<div class="col-sm-10">
									<c:forEach items="${perms}" var="perm" varStatus="vs">
										<div class="checks">
											<div>
												<div class="checkbox checkbox-inline checkbox-primary">
													<input id="parent-${vs.count}" type="checkbox" <c:if test="${perm.allCheck}">checked="checked"</c:if> />
													<label for="parent-${vs.count}"><strong>${perm.sysPermType.typeName}</strong></label>
												</div>
											</div>
											<div>
												<c:forEach items="${perm.perms}" var="permDto">
													<div class="checkbox checkbox-inline checkbox-primary">
														<input id="child-${permDto.sysPerm.id}" type="checkbox" name="ids" <c:if test="${permDto.checkBox}">checked="checked"</c:if>
														value="${permDto.sysPerm.id}" /> <label for="child-${permDto.sysPerm.id}">${permDto.sysPerm.permissionsName}</label>
													</div>
												</c:forEach>
											</div>
										</div>
		...
		<script>
			...
			$('#save-btn').on('click', function() {
				$.ajax({
					url: '${baseUrl}/admin/role/perm/allot',
